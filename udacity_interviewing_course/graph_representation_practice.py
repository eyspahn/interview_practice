...
class Graph(object):
    ...
    def get_edge_list(self):
        ...
        edge_list = []
        if self.edges not None:
            for elem in self.edges:
                edge_list.append((elem.value, elem.node_from, elem.node_to))
            return edge_list
        return []
    ...
    def get_adjacency_list(self):
        ...
        (To Node, Edge Value)"""
        max_val = self.find_max_value()
        adj_list = [None] * max_val
        if self.edges not None:
            for elem in self.edges:
                if adj_list[elem.node_from]:
                    adj_list[elem.node_from].append((elem.node_to, elem.value))
                else:
                    adj_list[elem.node_from] = [(elem.node_to, elem.value)]
        return adj_list
